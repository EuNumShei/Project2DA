...
void Rede::ler_ficheiro_navios() {
    ifstream in("../Toy-Graphs/shipping.csv");
    if (!in) {
        ...
        while (getline(in, linha)) {
            ...
            while (std::getline(iss, palavra, ',')) {
                switch (count) {
                    case 0:
                        ...
                        break;
                    case 1:
                        destino = stoi(palavra);
                        break;
                    case 2:
                        distancia = stod(palavra);
                        break;
                    default:
                        break;
                }
            ...
            if (shipping.findVertex(origem) == nullptr) {
                shipping.addVertex(origem);
            }
            if (shipping.findVertex(destino) == nullptr) {
                shipping.addVertex(destino);
            }
            shipping.addEdge(origem, destino, distancia);
            /*
...
void Rede::ler_ficheiro_estadios() {
    ifstream in("../Toy-Graphs/stadiums.csv");
    if (!in) {
        cerr << "Erro ao abrir o arquivo." << endl;
    } else {
        string linha;
        ...
        while (getline(in, linha)) {
            ...
            if (stadiums.findVertex(origem) == nullptr) {
                stadiums.addVertex(origem);
            }
            if (stadiums.findVertex(destino) == nullptr) {
                stadiums.addVertex(destino);
            }
            stadiums.addEdge(origem, destino, distancia);
            /*
...
void Rede::ler_ficheiro_turismo() {
    ifstream in("../Toy-Graphs/tourism.csv");
    if (!in) {
        cerr << "Erro ao abrir o arquivo." << endl;
    } else {
        string linha;
        ...
        while (getline(in, linha)) {
            ...
            if (tourism.findVertex(origem) == nullptr) {
                tourism.addVertex(origem);
            }
            if (tourism.findVertex(destino) == nullptr) {
                tourism.addVertex(destino);
            }
            tourism.addEdge(origem, destino, distancia);
            /*
...
void Rede::ler_ficheiro_graph(int num, bool real_graph) {
    string path;
    string edges_path;
    string nodes_path;

    std::vector<int> lista = {25, 50, 75, 100, 200, 300, 400, 500, 600, 700, 800, 900};
    auto it = std::find(lista.begin(), lista.end(), num);

    if (num < 4 && num > 0 && real_graph) {
        path = "../Real-world Graphs/graph" + to_string(num);
        edges_path = path + "/edges.csv";
        nodes_path = path + "/nodes.csv";
    } else if (it != lista.end() && !real_graph){
        path = "../Extra_Fully_Connected_Graphs";
        edges_path = path + "/edges_" + to_string(num) + ".csv";
        nodes_path = path + "/nodes.csv";
    } else {
    ...
    if (edges_path != "" && nodes_path != "") {
        ifstream in(edges_path);
        if (!in) {
            cerr << "Erro ao abrir o arquivo (edges)." << endl;
        } else {
            string linha;
            getline(in, linha);
            while (getline(in, linha)) {
                ...
                if (g.findVertex(origem) == nullptr) {
                    g.addVertex(origem);
                }
                if (g.findVertex(destino) == nullptr) {
                    g.addVertex(destino);
                }
                g.addEdge(origem, destino, distancia);
            }
        }

        ifstream in2(nodes_path);
        if (!in2) {
            cerr << "Erro ao abrir o arquivo (nodes)." << endl;
        } else {
            string linha2;
            getline(in2, linha2);
            while (getline(in2, linha2)) {
                ...
                double longitude;
                int count = 0;
                while (std::getline(iss2, palavra, ',')) {
                    switch (count) {
                        case 0:
                            id = stoi(palavra);
                            break;
                        case 1:
                            latitude = stod(palavra);
                            break;
                        default:
                            longitude = stod(palavra);
                            break;
                    }
                    count++;
                }
                ...
                if (v != NULL) {
                    v->setCoordenadas(latitude, longitude);
                }
            }
        }
    }
...
void Rede::print_edges_stadiums() {
    std::cout <<stadiums.getVertexSet().size() << std::endl;
    for (auto vertex : stadiums.getVertexSet()) {
        cout << vertex->getInfo() << ": " << "outedges:";
...
void Rede::print_edges_shipping() {
    std::cout <<shipping.getVertexSet().size() << std::endl;
    for (auto vertex : shipping.getVertexSet()) {
        cout << vertex->getInfo() << ": " << "outedges:";
        for (auto &edge : vertex->getAdj()) {
            cout << edge->getDest()->getInfo() << " ";
        }
...
void Rede::print_edges_tourism() {
    std::cout <<tourism.getVertexSet().size() << std::endl;
    for (auto vertex : tourism.getVertexSet()) {
        cout << vertex->getInfo() << ": " << "outedges:";
        for (auto &edge : vertex->getAdj()) {
            cout << edge->getDest()->getInfo() << " ";
        }
...
void Rede::print_edges_graph() {
    std::cout <<g.getVertexSet().size() << std::endl;
    for (auto vertex : g.getVertexSet()) {
        cout << vertex->getInfo() << ": " << "outedges:" << std::endl;
        for (auto &edge : vertex->getAdj()) {
            cout << edge->getDest()->getInfo() << " ";
            cout << edge->getDest()->getLatitude() << " ";
            cout << edge->getDest()->getLongitude() << " " << std::endl;
        }
        cout << endl;
    }
}
...
bool Rede::verificar_edge(const int& source, const int& dest) {
